...
import { parseRequestSource,sourceDescription } from './request-source';
...
export async function activityFor(uid:string, now=Date.now()) {
 ...
 SELECT e.*,
 (SELECT COUNT(DISTINCT b.message_id) FROM scoped b WHERE b.kind=e.kind AND b.event_type=e.event_type AND b.received_at BETWEEN e.received_at-1000 AND e.received_at+1000) AS burst_messages,
...
export function describeActivity(e:any,now=Date.now()) {
 ...
 if(source)source.assessment=sourceDescription(source,e.kind);
 const click=e.event_type==='click',sourceType=source?.assessment?.type;
 ...
 if(click){category=source?.fetchUser==='?1'?'Browser link visit':'Link request';reason='A tracked link was requested. Browser navigation is stronger evidence than an image load, but scanners and forwarded links can still generate requests.';}
 ...
 else if(!click&&sourceType==='privacy_relay'){category='Privacy relay image load';reason='Apple relay network. Mail privacy can load images before anyone reads the email.';excluded=true;}
 else if(sourceType==='security_scanner'){category='Likely security service';reason='Request came from a recognized email-security network. Retained for review; not counted as engagement.';excluded=true;}
 ...
 else if(!click&&age<=10000){category='Immediate image load';reason='Arrived within 10 seconds of send confirmation. A genuine fast open is possible, but this is excluded from the main activity count.';excluded=true;}
 else if(e.kind==='unknown_client'||(source?.version===2&&source.client==='Mozilla/5.0')){category='Unrecognized request';reason='The request did not identify a browser or known image proxy. It is retained for review, not counted as engagement.';excluded=true;}
...
export function summarizeActivity(events:any[]) {
 const usable=events.filter(e=>!e.excluded&&e.event_type!=='click');
 const clicks=events.filter(e=>!e.excluded&&e.event_type==='click');
 ...
 return {first_request:events[0]?.received_at??null,last_request:events[events.length-1]?.received_at??null,requests:events.length,loads:events.filter(e=>e.event_type!=='click').length,clickRequests:events.filter(e=>e.event_type==='click').length,clicks:clicks.length,last_click:clicks.at(-1)?.received_at??null,opens:sessions.length,uncertain:events.filter(e=>e.excluded).length,first_open:usable[0]?.received_at??null,last_open:usable[usable.length-1]?.received_at??null};
}
