...
export function agentInsights(s:Snapshot) {
 ...
 {title:opens>unique_opens?'Repeat image loads detected':'Your data stays focused',text:opens>unique_opens?(opens-unique_opens)+' additional image loads followed first loads in this period. These may include repeat views, forwarding, or privacy services.':'The agent records the time and request category. Source details include the request IP and approximate network location when available; proxies can hide the reader.',type:'context'}
 ];
}
