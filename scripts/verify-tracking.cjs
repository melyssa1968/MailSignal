...
(async()=>{
 ...
 list=await (await emailsRoute.GET(request('/api/emails',undefined,'GET'))).json();assert.equal(list.emails.find(e=>e.id===burst[0]).opens,1);
 let t=await (await timelineRoute.GET(request('/api/messages/'+burst[0]+'/events',undefined,'GET'),ctx(burst[0]))).json();assert.equal(t.events.filter(e=>!e.excluded).length,2);
