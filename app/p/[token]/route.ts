...
export async function GET(request:Request,{params}:{params:Promise<{token:string}>}){
 ...
 try{
 const m=await db().prepare('SELECT m.email,c.owner FROM messages m JOIN campaigns c ON c.id=m.campaign_id WHERE m.id=?').bind(id).first<{email:string;owner:string}>();
 if(!m||isExcluded(m.email,(await settingsFor(m.owner)).excludedDomains))return pixel();
 await db().prepare(`INSERT INTO events(id,message_id,received_at,kind) SELECT ?,m.id,?,? FROM messages m JOIN campaigns c ON m.campaign_id=c.id WHERE m.id=? AND c.status='active' AND (m.sent_at IS NOT NULL OR m.sending_at IS NOT NULL)`)
 .bind(crypto.randomUUID(),Date.now(),classifyRequest(request.headers.get('user-agent')||''),id).run();
