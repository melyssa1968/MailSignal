...
export async function GET(r:Request,{params}:{params:Promise<{id:string}>}){try{
 ...
 const e=await db().prepare('SELECT id,received_at,kind FROM events WHERE message_id=? ORDER BY received_at ASC,id ASC LIMIT 501').bind(id).all<any>();
 return json({subject:m.subject,recipients:recipientsFor(m),sentAt:m.sent_at,truncated:e.results.length>500,events:e.results.slice(0,500).map(v=>({...v,category:signalCategory(v.kind,v.received_at,m.sent_at),reason:signalReason(v.kind,v.received_at,m.sent_at),secondsAfterSend:(v.received_at-m.sent_at)/1000}))});
