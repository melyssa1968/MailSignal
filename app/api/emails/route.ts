...
export async function GET(r:Request){try{
 ...
 return json({emails:result.results.filter(m=>eligibleRecipients(m,s.excludedDomains).length).map(m=>({...m,recipients:recipientsFor(m),...summarizeActivity(byMessage.get(m.id)||[]),latestSource:(byMessage.get(m.id)||[]).at(-1)?.source??null})),settings:s,refreshedAt:Date.now()});
